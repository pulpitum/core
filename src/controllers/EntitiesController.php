...
use URL;
use View;
/*use Pulpitum\Datatables\Models\UsersCache;*/

class EntitiesController extends BackendController {
    ...
    public function getEdit($id){
    	...
    	return $this->theme->of('core::forms.edit', array("model" => $model, "update_rules"=>$this->rules, "create_rules"=>$this->rules, "title"=>$this->getName(), "entity"=>$this->entidade, "return"=>$return['list']['as'] ))->render();
    }
    ...
    public function postEdit($id){
        ...
        if($validator->fails())
        {
            Session::flash('warning', trans('core::form.error')."<br />".$validator->messages());
		...
		try{
			if( $entidade->save() )
			{
	            Session::flash('success', trans('core::form.save-success'));
	            if(Input::get("back")!="")
	            	return Redirect::to(url(Input::get("back")));
            	$return = Input::get("return");
				return Redirect::route($return);
			}else{
	            Session::flash('warning', trans('core::form.try'));
		    	return Redirect::to(URL::previous())->withInput();
			}
		}catch(\Illuminate\Database\QueryException $e){
	            Session::flash('warning', trans('core::form.error'));
		    	return Redirect::to(URL::previous())->withInput();
		}
    }
    ...
    public function getAdd(){
    	$model = $this->entidade;
    	$return = $this->entidade->actionsUrl();
    	return $this->theme->of('core::forms.edit', array("model" => $model,"update_rules"=>$this->rules, "create_rules"=>$this->rules, "title"=>$this->getName(), "entity"=>$this->entidade, "return"=>isset($return['list']['as']) ? $return['list']['as'] : "" ))->render();
    }
    ...
    public function getAjaxAdd(){
    	$model = $this->entidade;
    	$return = $this->entidade->actionsUrl();
    	return View::make('core::forms.ajax', array("model" => $model,"update_rules"=>$this->rules, "create_rules"=>$this->rules, "title"=>$this->getName(), "entity"=>$this->entidade, "return"=>isset($return['list']['as']) ? $return['list']['as'] : "" ))->render();
    }
    ...
    function postAjaxAdd(){
		...
		try{
			if( $entidade->save() )
			{
		        $response = array(
		            'status' => 'success',
		            'msg' => trans('core::form.save-success'),
		        );
			}else{
		        $response = array(
		            'status' => 'error',
		            'msg' => trans('core::form.try'),
		        );
			}
		}catch(\Illuminate\Database\QueryException $e){
	        $response = array(
